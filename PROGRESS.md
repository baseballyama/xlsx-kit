...
- **フェーズ**: フェーズ2 (コアモデル)
- **次のタスク**: フェーズ2 §6 JSON round-trip 統合テスト。`createWorkbook → addWorksheet → setCell × 複数 → setCellFont → mergeCells (※ §5 mergedCells は phase 5 へ deferred なので最低限 cellRange の往復のみ)` を `JSON.stringify(wb, jsonReplacer) → JSON.parse(json, jsonReviver)` で復元し、cell value / styleId / Map keys / Stylesheet pool / NamedStyle が同一性を保つことを検証。次いで §7 phase-2 受け入れ条件確認 → フェーズ3 へ。
