...
- **次のタスク**: **custom-property ergonomic helpers (`setCustomStringProperty` / `Number` / `Bool` / `Date` / `getCustomPropertyValue` / `removeCustomProperty` / `listCustomProperties`) を追加**。docProps/custom.xml を JS 値で直接編集:
