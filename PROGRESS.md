...
- **フェーズ**: フェーズ1（基盤層）
- **次のタスク**: フェーズ1 §6 packaging 層の続き：`docProps/core.xml` (`CoreProperties`)。Dublin Core の小さな schema（title, subject, creator, keywords, description, lastModifiedBy, modified, created, category, contentStatus, identifier, language, version, revision, lastPrinted）。dcterms 名前空間と `xsi:type="dcterms:W3CDTF"` の datetime 属性に注意。`docProps/app.xml` (Extended) と `docProps/custom.xml` も合わせる予定だが、まずは core を一発。openpyxl genuine/empty.xlsx の docProps/core.xml で round-trip。
...
- [~] §6 packaging 層（manifest + relationships 完了：`Manifest` (defaults + overrides の sequence schema、CONTYPES_NS) と `Relationships` (rels の sequence schema、PKG_REL_NS) を Schema 経由で round-trip。`makeManifest` / `addDefault` / `addOverride` / `findOverride` / `findOverrideByContentType` / `manifestTo`/`FromBytes`、`makeRelationships` / `appendRel`（最小未使用 rId 割り当て、external targetMode）/ `findById` / `findByType` / `findAllByType` / `relsTo`/`FromBytes`。openpyxl `genuine/empty.xlsx` の `[Content_Types].xml` / `_rels/.rels` / `xl/_rels/workbook.xml.rels` がすべて round-trip。157 tests pass。残：docProps/core.xml + app.xml + custom.xml）
