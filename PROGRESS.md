...
- **フェーズ**: フェーズ2 (コアモデル)
- **次のタスク**: フェーズ2 §6 JSON round-trip 統合テスト。`createWorkbook → addWorksheet → setCell × 複数 → setCellFont → cellRange を sqref 経由で保持` を `JSON.stringify(wb, jsonReplacer) → JSON.parse(json, jsonReviver)` で復元し、cell value / styleId / Map keys / Stylesheet pool / NamedStyle が同一性を保つことを検証 (§3.6 ブリッジ完了済みなので setCellFont 経由で styleId が dedup されることを確認できる)。次いで §7 phase-2 受け入れ条件確認 → フェーズ3 へ。
