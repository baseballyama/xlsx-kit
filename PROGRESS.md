...
- **フェーズ**: フェーズ1（基盤層）
- **次のタスク**: フェーズ1 §6 packaging 層の続き：`docProps/app.xml` (Extended Properties) と `docProps/custom.xml`。app.xml は `<Application>` / `<DocSecurity>` / `<Company>` / `<AppVersion>` 等の単純フィールドはフル schema、`<HeadingPairs>` / `<TitlesOfParts>` の `vt:vector` だけ raw XML 保持で round-trip。custom.xml は `<property fmtid pid name>` に `<vt:lpwstr>`/`<vt:i4>`/`<vt:filetime>` 等の値が入るパターン、こちらも openpyxl 互換のフィールドだけ schema 化して残りは passthrough。
...
- [~] §6 packaging 層（manifest + relationships + docProps/core.xml 完了：Schema の text 種別に固定 `attrs` を追加して `xsi:type="dcterms:W3CDTF"` を表現できるようにし、`CoreProperties` (cp/dc/dcterms namespace 混合の text 群、15 フィールド全て optional) を schema 化。`corePropsTo`/`FromBytes`、`makeCoreProperties`。openpyxl `genuine/empty.xlsx` の `docProps/core.xml` で creator/lastModifiedBy/created/modified の値一致 + 完全 round-trip。163 tests pass。残：docProps/app.xml + docProps/custom.xml）
