...
- **フェーズ**: フェーズ2 (コアモデル)
- **次のタスク**: フェーズ2 §5 Formula tokenizer / translator (openpyxl `formula/tokenizer.py` / `translate.py` 移植)。Tokenizer は state machine + regex (LITERAL / FUNC / SEP / OP / WSPACE / RANGE / NUM / TEXT 等)、Translator は shared formula の reference shifting（A1↔R1C1 変換 + 絶対/相対 anchor 保持）。**数式評価は絶対にしない**。次いで §6 JSON round-trip 統合テスト、§7 phase-2 受け入れ条件確認 → フェーズ3 へ。
