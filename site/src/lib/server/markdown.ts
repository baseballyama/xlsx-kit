...
Working code for the things people actually want to do with ooxml-js — open a workbook, build one from scratch, style cells, add a chart, stream millions of rows. Every snippet below is a real \`.ts\` file in the repo, type-checked against \`ooxml-js\` on every build.
`;
