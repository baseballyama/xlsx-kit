// Cell-level error / watch metadata. Per docs/plan/13-full-excel-coverage.md
// §B8 (cellWatches / ignoredErrors).
//
// `ignoredErrors` lets you tell Excel "don't flag these cells with the
// little green triangle for this class of validation". `cellWatches`
// records which cells the user has pinned in the Watch Window.
//
// Both round-trip via the worksheet `bodyExtras` passthrough already,
// but promoting them to first-class arrays gives the editor a clean
// API and the writer a stable position in the worksheet element order
// (between rowBreaks / colBreaks and the drawing block per ECMA-376
// §18.3.1.94 cellWatches / §18.3.1.51 ignoredErrors).

import type { MultiCellRange } from './cell-range';

/**
 * One Cell-Watch entry. The Watch Window in Excel (Formulas → Watch
 * Window) shows live values for the cells listed here.
 */
...
/**
 * One ignored-error entry. Each Boolean flag corresponds to a class of
 * Excel's background validation; setting any of them to `true`
 * suppresses the green triangle for cells in `sqref`.
 */
