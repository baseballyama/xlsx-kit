# Migrating from openpyxl to xlsxlite

`xlsxlite` is a TypeScript port of openpyxl. The data model, naming, and
...
```ts
// xlsxlite
import { fromFile, loadWorkbook, saveWorkbook, toFile } from 'xlsxlite/node';
...
that the cell points at via a styleId. xlsxlite keeps the same model — the
...
xlsxlite doesn't re-implement every OOXML schema; instead, parts that
...
If openpyxl preserves it, `xlsxlite` does too — but typically without a
