...
`openpyxl`（Python, MIT, 3.1.5 系）を TypeScript に移植し、**Node 18+ / 主要モダンブラウザ** の両方で動作する OOXML (xlsx/xlsm) 操作ライブラリ `xlsxlite` を作るための、フェーズ分割された実装計画ドキュメントです。
...
| 02 | [02-mapping.md](./02-mapping.md) | openpyxl Python モジュール ↔ xlsxlite TS モジュールの対応表 |
