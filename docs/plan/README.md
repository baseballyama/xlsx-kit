...
`openpyxl`（Python, MIT, 3.1.5 系）を TypeScript に移植し、**Node 22+ / 主要モダンブラウザ** の両方で動作する OOXML (xlsx/xlsm) 操作ライブラリ `xlsx-kit` を作るための、フェーズ分割された実装計画ドキュメントです。
...
| 02 | [02-mapping.md](./02-mapping.md) | openpyxl Python モジュール ↔ xlsx-kit TS モジュールの対応表 |
