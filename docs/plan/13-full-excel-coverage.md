...
| B6 | **Print Settings** (rowBreaks / colBreaks / pageSetup / headerFooter / printOptions) | 🟡 部分対応 | `<printOptions>` (5 boolean) / `<pageMargins>` (6 float) / `<pageSetup>` (paperSize / scale / orientation / fitToWidth/Height / DPI / pageOrder / cellComments / errors etc.) / `<headerFooter>` (4 flag + 6 mini-format strings) は typed API + 完全 round-trip (`src/worksheet/page-setup.ts`、`tests/phase-5/page-setup.test.ts` 5 件、e2e 23 が `makePageSetup` etc. を実装)。残：rowBreaks / colBreaks の typed API | 2 |
