...
**Excel が「ネイティブに」できる全機能** を ooxml-js で読み書きできる
...
| B5 | **Workbook / Sheet protection** | 🟡 部分対応 | `<sheetProtection>` 全 16 boolean lock flag + 4 password-hash field (`src/worksheet/protection.ts`)、`<workbookProtection>` 全 15 attr (modern hash quad × 2 + legacy 16-bit hex × 2 + lockStructure / lockWindows / lockRevision、`src/workbook/protection.ts`、`tests/phase-3/workbook-protection.test.ts` 4 件) を typed API + round-trip 完了。残：パスワード hash 計算 helper (legacy + SHA-512) | 2 |
| B6 | **Print Settings** (rowBreaks / colBreaks / pageSetup / headerFooter / printOptions) | ✅ | `<printOptions>` (5 boolean) / `<pageMargins>` (6 float) / `<pageSetup>` (paperSize / scale / orientation / fitToWidth/Height / DPI / pageOrder / cellComments / errors etc.) / `<headerFooter>` (4 flag + 6 mini-format strings) / `<rowBreaks>` / `<colBreaks>` 全て typed API + 完全 round-trip (`src/worksheet/page-setup.ts`、`tests/phase-5/page-setup.test.ts` 6 件、e2e 23 が manual page break も含めた full setup を実証) | 2 |
| B7 | **Sheet view** 拡張 (sheetPr / customSheetViews / scenarios / dataConsolidate) | 🟡 部分対応 | sheetPr (`src/worksheet/properties.ts`) / dataConsolidate (`src/worksheet/data-consolidate.ts`) / scenarios (`src/worksheet/scenarios.ts` — Scenario Manager: ScenarioList { scenarios, current, show, sqref } + Scenario { name, locked, hidden, user, comment, inputCells } + ScenarioInputCell { ref, val, deleted, undone, numFmtId }) は全て typed API + round-trip 完了。残：customSheetViews | 1–2 |
...
| B9 | **Web Publish Items / Custom Properties (worksheet level)** | ✅ | `<customProperties>` (name + r:id; rels link survives via `relsExtras`) と `<webPublishItems>` (id / divId / sourceType enum / destinationFile + 4 optional attr) を typed API + 完全 round-trip (`src/worksheet/web-publish.ts`、`tests/phase-5/web-publish.test.ts` 3 件) | 0.5 |
| B10 | **Phonetic / EaList (East Asian features)** | 🟡 部分対応 | worksheet-level `<phoneticPr>` (fontId + type 4-enum + alignment 4-enum) を typed API + 完全 round-trip (`src/worksheet/phonetic.ts`、`tests/phase-5/phonetic.test.ts` 3 件)。残：per-cell `<rPh sb=… eb=…>` (rich text 内の furigana annotations) | 1 |
...
| F9 | Bundle 構成 (`ooxml-js/streaming` など) | ✅ | サブパス追加ごとに更新 |
...
5. **bundle 予算を破らない**: 機能を増やすときは必ず subpath で分離。`ooxml-js` (full) ≤ 200 KB / `ooxml-js/streaming` ≤ 80 KB / 大規模機能は `ooxml-js/pivot` `ooxml-js/powerquery` 等の独立サブパス。
