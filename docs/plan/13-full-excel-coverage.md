...
**Excel が「ネイティブに」できる全機能** を xlsx-kit で読み書きできる
...
| F9 | Bundle 構成 (`xlsx-kit/streaming` など) | ✅ | サブパス追加ごとに更新 |
...
5. **bundle 予算を破らない**: 機能を増やすときは必ず subpath で分離。`xlsx-kit` (full) ≤ 200 KB / `xlsx-kit/streaming` ≤ 80 KB / 大規模機能は `xlsx-kit/pivot` `xlsx-kit/powerquery` 等の独立サブパス。
