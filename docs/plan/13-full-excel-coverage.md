...
**Excel が「ネイティブに」できる全機能** を xlsxlite で読み書きできる
...
| F9 | Bundle 構成 (`xlsxlite/streaming` など) | ✅ | サブパス追加ごとに更新 |
...
5. **bundle 予算を破らない**: 機能を増やすときは必ず subpath で分離。`xlsxlite` (full) ≤ 200 KB / `xlsxlite/streaming` ≤ 80 KB / 大規模機能は `xlsxlite/pivot` `xlsxlite/powerquery` 等の独立サブパス。
