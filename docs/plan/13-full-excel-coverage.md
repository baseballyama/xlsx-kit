...
| B7 | **Sheet view** 拡張 (sheetPr / customSheetViews / scenarios / dataConsolidate) | 🟡 部分対応 | sheetPr 全体 (codeName / tabColor / outlinePr / pageSetUpPr 含む全 9 attr + 3 child) は `src/worksheet/properties.ts` で typed API + round-trip 完了 (e2e 27 が `makeSheetProperties` 経由で tab color を着色)。dataConsolidate (function 11-enum + 3 boolean + dataRefs[] with name/ref/sheet/rId) は `src/worksheet/data-consolidate.ts` で typed API + round-trip 完了 (`tests/phase-5/data-consolidate.test.ts` 3 件)。残：customSheetViews / scenarios | 1–2 |
