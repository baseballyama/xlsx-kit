...
Node 22+ は `ReadableStream`/`WritableStream` をネイティブで提供する。xlsx-kit の **ストリーミング API は Web Streams を共通インターフェース** として採用し、Node 専用の `Readable`/`Writable` は `Readable.toWeb()` で橋渡しする。
...
- 内部例外は `cause` でチェーン。Node 22+ / モダンブラウザは `Error.cause` をサポート。
