...
`package.json` の `exports` 条件でこれらを `ooxml-js/io` のサブパスから両環境が同名 import できるようにする。Node 専用の `fromFile(path)` とブラウザ専用の `fromFile(File)` は **シグネチャ重複でも実体は別ファイル**で ok。
...
- [ ] `defineSchema` が tree-shake で不要な schema を除去できる（テスト: ダミー entry で `import { foo } from 'ooxml-js/styles'` 後の bundle に未使用 schema が含まれないこと）
