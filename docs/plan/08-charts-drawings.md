...
**目的**: DrawingML と ChartML を **フル構造化**。とくに ChartML は **「Excel でできることを xlsxify でも全部できる」** をゴールとする最重要モジュール。
**期間目安**: 8〜12週間（最重量フェーズ）
**前提**: フェーズ1〜5
...
> **このフェーズだけは「passthrough で逃げない」**。Excel でできるチャート機能は xlsxify でも作れる、編集できる、再現できるのが目標。
...
差分があれば **Excel 365 を正** とする（openpyxl が拾えていない属性は xlsxify 側で拾う）。
...
- [ ] Excel で chartex 図を作って xlsxify で読んでも seriesData が消えない
...
- [ ] chart 上に矢印を引く編集が xlsxify で可能、かつ Excel で問題なく開ける
...
- [ ] 既存フェーズの回帰なし
- [ ] バンドルサイズ予算: `xlsxify/chart` は ChartML の量が多いため特例で **≤ 120KB min+gz**（基本サブセット）。`xlsxify/chart/extended` で chartex を分離して **≤ 60KB min+gz** 追加。`xlsxify/drawing` ≤ 40KB min+gz
